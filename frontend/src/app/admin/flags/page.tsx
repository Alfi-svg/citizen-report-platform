...
export default function AdminFlagsPage() {
  ...
  }, [isAuthenticated, isAdmin, page, targetTypeFilter, statusFilter]);

  const handleUpdateStatus = async (flagId: string, newStatus: FlagStatus) => {
    try {
      const updated = await apiFetch<AdminFlag>(`/admin/flags/${flagId}`, {
        method: "PATCH",
      ...
      setActionSuccess(`Flag status updated to ${newStatus}.`);
    } catch (err: unknown) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
            <span>🚩 Safety & Content Flag Queue</span>
            <span className="rounded-full bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300 text-xs px-2.5 py-0.5 font-bold">
              {total} Total
            </span>
          </h1>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-zinc-100 dark:border-zinc-800">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {flag.status !== "ACTION_TAKEN" && (
                      <button
                        type="button"
                        onClick={() => handleUpdateStatus(flag.id, "ACTION_TAKEN")}
                    ...
                    {flag.status !== "DISMISSED" && (
                      <button
                        ...
                        className="rounded-lg border border-zinc-300 dark:border-zinc-700 px-3 py-1.5 text-xs font-semibold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50"
                      >
                        ✕ Dismiss Flag
                      </button>
                    )}
                    {flag.status !== "REVIEWED" && (
                      <button
                        ...
                        className="rounded-lg border border-blue-300 dark:border-blue-800 px-3 py-1.5 text-xs font-semibold text-blue-700 dark:text-blue-300 hover:bg-blue-50"
                      >
                        👁️ Mark Reviewed
                      </button>
                    )}
